...
const UkokoPenerbitanForm: React.FC<{ deptName: string; onBack: () => void; readOnly?: boolean }> = ({ deptName, onBack, readOnly = false }) => {
  const {
    formData,
    setFormData,
    handleSave,
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-gradient-to-br from-emerald-900 to-green-800 p-6 rounded-3xl shadow-lg text-white flex flex-col justify-center relative overflow-hidden group">
          <div className="relative z-10">
            <p className="text-emerald-300 text-[10px] font-black uppercase tracking-widest mb-1 font-sans">Jumlah Penerbitan 2025</p>
          ...
          <table className="w-full min-w-[420px] text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-[10px] font-black uppercase text-emerald-900">Input 2025</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              <tr>
                ...
                <td className="px-4 py-3 text-center text-sm font-black text-emerald-700">{totalPublications}</td>
              </tr>
            </tbody>
          </table>
        </div>
                ...
                <tr key={idx} className="group hover:bg-emerald-50/30 transition-colors">
                  ...
                  <td className="px-4 py-4">
                    <select
                      ...
                      disabled={readOnly}
                      className="w-full bg-transparent border-none focus:ring-0 text-sm font-bold text-emerald-600"
                    >
              ...
              {(formData.penerbitan?.senaraiPenerbitan || []).length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-12 text-center text-gray-400 text-sm font-medium italic bg-gray-50 rounded-b-2xl">
                    Tiada rekod penerbitan ditambah.
                  </td>
                </tr>
              )}
            </tbody>
